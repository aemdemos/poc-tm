...
### Page Visual Compare (original vs new at all resolutions)

**When a user wants to compare an original page with a new/migrated version and get a clear verdict plus actionable fixes**, use the Page Visual Compare skill. It captures both pages at multiple desktop and mobile viewports, determines if they are visually identical, and if not, lists differences and recommends CSS/layout changes so the new version matches the original at all resolutions.

**Trigger patterns:**
- User says: "compare original and new page", "check if page is identical", "page comparison", "visual diff", "what's different between original and migrated", "make new page match original", "validate page at all resolutions" → invoke directly.

**How to invoke:**
Read and follow `.claude/skills/excat-page-visual-compare/SKILL.md`. Provide original URL or path and new URL or path; capture screenshots at each viewport; compare; write `comparison-work/page-visual-compare/{runId}/comparison-report.json` with `identical`, per-viewport `differences`, and `recommendedChanges`.

**Do NOT use for:** Header-only or single-component comparison (use Navigation Orchestrator or nav-component-critique).